...
- Polish Text: `✨ AI` "Привести в порядок" во вкладке диктовки (Ollama-only)
...
- История транскрибации (сворачиваемый блок) с экспортом ZIP
...
- в истории транскрибации кнопка `Export ZIP` (в архиве `note.md` + `transcript.txt`)
  - имя ZIP формируется из заголовка: `safe_slug(title) + timestamp`
- в истории транскрибации есть кнопка `🗑`:
  - удаляет запись из архива
  - удаляет физические файлы job из `output/...`
...
В `transcript.json` дополнительно сохраняется:

- `meta.title` — краткий заголовок записи
  - генерируется один раз при создании job
  - источник: summary (если есть), иначе начало транскрипта
  - Ollama (локально) при доступности, иначе fallback-эвристика
...
## Hugging Face Access Tokens (Pyannote)

Нужно только если вы используете gated/private модели Hugging Face (например, `pyannote` для diarization).
Для стандартного сценария TranscribeLite без pyannote этот шаг не обязателен.

1. Войдите в аккаунт Hugging Face: `https://huggingface.co/login`
2. Создайте токен (`Read`): `https://huggingface.co/settings/tokens`
3. Подтвердите доступ на страницах моделей pyannote:
   - `https://huggingface.co/pyannote/segmentation-3.0`
   - `https://huggingface.co/pyannote/speaker-diarization-3.1`
4. На каждой странице нажмите кнопку доступа/подтверждения условий (`Access repository` / `Agree and access`).

Настройка токена в Windows:

- На текущую сессию `cmd`:
  - `set HF_TOKEN=hf_your_token_here`
- Постоянно (в новых окнах `cmd`):
